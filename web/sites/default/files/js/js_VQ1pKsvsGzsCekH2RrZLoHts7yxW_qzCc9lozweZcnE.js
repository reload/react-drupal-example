...
(function (Drupal, drupalSettings) {
  Drupal.behaviors.reactExample = {
    attach: function (_context, _settings) {
      ...
      document.querySelectorAll('.react-example').forEach(node => {
        ReactDOM.render(html`<${App} initialName="Drupal" />`, node)
      })
    }
  };
}(Drupal, drupalSettings));
...
function App ({ initialName }) {
  ...
  return html`
    <div>
      <${Title} name=${name} />
      ${name === initialName && (
        html`<button onClick=${() => setName('React')}>Press me to draw!</button>`
      )}
      ${name === 'React' && (
        html`
